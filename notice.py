...
class notice(ttk.Frame):
    ...
    def __init__(self, parent, controller, log):
        ...
        if self.log[2] == '관리자' or self.log[2] == '경비원':
            ttk.Button(self, width=30, text='글 작성', style="TButton",
                       command=lambda: controller.controller.show_frame("write_notice")).place(x=1300, y=100)
    ...
    def show(self):
        ...
        self.controller.controller.cur.execute('select * from 공지')
    ...
    def OnDoubleClick(self, event):
        for F in (talkingroom.talkingRoom,):
            ...
            frame = F(parent=self.controller.controller.container, controller=self, log=self.log,
                      data=self.treeview.item(self.treeview.selection()[0], 'values'), class_=notice.__name__)
